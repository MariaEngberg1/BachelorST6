...
void setup() {
  ...
  for (int j = 0; j < arraySize; j++) {
    readings[j] = 0;                                   // initialize all the readings to 0
    scale[j] =  floatMap(j, 0, (arraySize), 100, 1);   // find all scale factors
  } 
}
...
void setup_timer() {
  ...
  timerAlarmWrite(timer, 5000, true);            // set timer value to 5000 x 1 micro sec = 5 ms -> Fs = 200 Hz, true = reload
...
float Poly41(float x, float y){                 // returns force in newton  
...
void loop() {
  ...
  if(doDataCollection){
    ...
    force = Poly41(readings[currentIndexNumber], integrel); 
    if( force < 5.33){
    ...
    currentIndexNumber ++; 
    if (currentIndexNumber == arraySize) {                 // wrap around to the beginning
